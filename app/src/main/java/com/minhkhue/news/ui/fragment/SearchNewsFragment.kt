...
import com.minhkhue.news.databinding.FragmentSearchNewsBinding
import com.minhkhue.news.ui.NewsActivity
import com.minhkhue.news.ui.viewmodel.NewsViewModel
import com.minhkhue.news.utils.Constants
...
class SearchNewsFragment : Fragment() {
	...
	override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
		...
		viewMode.searchNews.observe(viewLifecycleOwner, { response ->
			when (response) {
				is Resource.Success -> {
					hideProgressbar()
					response.data?.let { newsResponse ->
						...
						isLastPage = viewMode.searchNewsPage == totalPages
						if (isLastPage) {
				...
				is Resource.Error -> {
					...
					response.message?.let { message ->
						Toast.makeText(requireContext(), "An error: $message", Toast.LENGTH_SHORT)
							.show()
					}
				}
				is Resource.Loading -> {
	...
	private val scrollListener = object : RecyclerView.OnScrollListener() {
		override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
			...
			if (shouldPaginate) {
				viewMode.searchNews(binding.etSearch.text.toString())
				isScrolling = false
			}
		}
